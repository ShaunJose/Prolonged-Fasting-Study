...
baseline_data=select(bl_data,-matches("Weight|Waist|SBP|DBP|^Hb$|Ht|Neutr|Sodium|Potassium|Chloride|Calcium|ALT|Triglycerides|^HDL$"))
...
cluster_labels_df = data.frame(baseline_1_kmeans$cluster,grp1,grp2,grp3,grp4)
colnames(cluster_labels_df)=c("kmeans","hclust1","hclust2","hclust3","hclust4")
...
pvrect(baseline_1_pvclust1, alpha = 0.95)
plot(baseline_1_pvclust2)
pvrect(baseline_1_pvclust2, alpha = 0.95)
```
...
### PCA without BMI (only biomarkers)
res.pca <- prcomp(scaled.baseline_1[,-1])
# res.pca <- prcomp(scaled.baseline_1[,-c(1,2)]) if weight BMI included
# Visualize eigenvalues/variances
fviz_screeplot(res.pca, addlabels = TRUE, ylim = c(0, 50))
# Extract the results for variables
var = get_pca_var(res.pca)
...
res.pca <- prcomp(scaled.blef_1[,-c(1,2)])
# Visualize eigenvalues/variances
fviz_screeplot(res.pca, addlabels = TRUE, ylim = c(0, 50))
# Extract the results for variables
var = get_pca_var(res.pca)
...
### PCA without weight and BMI (only biomarkers)
res.pca <- prcomp(scaled.efer_1[,-c(1,2)])
# Visualize eigenvalues/variances
fviz_screeplot(res.pca, addlabels = TRUE, ylim = c(0, 50))
# Extract the results for variables
var = get_pca_var(res.pca)
